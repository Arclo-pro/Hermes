...
/**
 * Send team invitation email to a prospective team member
 */
...
/**
 * Send notification to account owner when an invited user joins their team
 */
export async function sendTeamJoinNotification(
  ...
  try {
    const { client, fromEmail } = getSendGridClient();
    const baseUrl = getBaseUrl();
    const teamUrl = `${baseUrl}/settings?tab=team`;

    const msg = {
      to: ownerEmail,
      from: fromEmail,
      subject: `${newMemberName} joined your Arclo team`,
      text: `Good news!\n\n${newMemberName} (${newMemberEmail}) has accepted your invitation and joined your Arclo team.\n\nView your team: ${teamUrl}\n\nBest,\nThe Arclo Team`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: linear-gradient(135deg, #10b981 0%, #06b6d4 100%); padding: 30px; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 28px;">Arclo</h1>
          ...
          <div style="padding: 40px 30px; background: #f9fafb;">
            <h2 style="color: #111827; margin-top: 0;">New team member!</h2>
            <p style="color: #4b5563; font-size: 16px; line-height: 1.6;">
              <strong>${newMemberName}</strong> (${newMemberEmail}) has accepted your invitation and joined your Arclo team.
            </p>
            <p style="color: #4b5563; font-size: 16px; line-height: 1.6;">
              They now have full access to all your websites and SEO tools.
            </p>
            <div style="text-align: center; margin: 30px 0;">
              <a href="${teamUrl}" style="background-color: #7c3aed; color: #ffffff; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block; font-size: 16px;">
                View Team
              </a>
            </div>
          </div>
    ...
    await client.send(msg);
    console.log(`[Email] Team join notification sent to ${ownerEmail}`);
    return true;
  } catch (error: any) {
    console.error('[Email] Failed to send team join notification:', error.message);
    return false;
  }
}
