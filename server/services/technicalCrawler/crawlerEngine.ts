...
import { normalizeUrl, isInternalUrl } from "./urlUtils";
import { parseHtml, determineIndexability } from "./htmlParser";
import { runFindingRules, type CrawlFinding, type FindingContext } from "./findingRules";
...
async function fetchSitemapUrls(
  ...
  while (queue.length > 0 && allUrls.length < maxUrls) {
    ...
    try {
      ...
      logger.info("TechnicalCrawler", `Failed to fetch sitemap ${sitemapUrl}`);
    }
  }
...
export async function runTechnicalCrawl(
  ...
  logger.info("TechnicalCrawler", `Starting crawl of ${domain} (max ${cfg.maxPages} pages)`);
  ...
  try {
    ...
    logger.info("TechnicalCrawler", "No robots.txt found");
  }
  ...
  logger.info("TechnicalCrawler", `Found ${sitemapUrlSet.size} sitemap URLs`);
  ...
  while (urlQueue.length > 0 && processedCount < cfg.maxPages) {
    ...
    if (processedCount % 10 === 0) {
      logger.info("TechnicalCrawler", `Crawled ${processedCount} / ${cfg.maxPages} pages`);
    }
  }
  ...
  for (const page of pages) {
    ...
    if (norm && sitemapUrlSet.has(norm) && !inlinkCounts.has(norm)) {
      // Already caught by RULE_ORPHAN_PAGE during crawl if inlinksCount was set
      // But orphan detection needs full inlink data which is only complete post-crawl
  ...
  logger.info("TechnicalCrawler", `Crawl complete: ${pages.length} pages, ${allFindings.length} findings in ${durationMs}ms`);
