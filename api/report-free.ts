  ...
  try {
    ...
    const result = await pool.query(
      `SELECT report_id, scan_id, website_url, website_domain, report_version, status,
       ...
       FROM free_reports WHERE report_id = $1`,
      [reportId]
    );
    ...
    if (report.status === "generating") {
      return res.status(202).json({ ok: true, status: "generating", message: "Report is still being generated" });
    }
    ...
    console.error("[ReportFree] Error:", error?.message);
    if (!res.headersSent) {
