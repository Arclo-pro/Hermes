...
export const servicesCatalog: ServiceDefinition[] = [
  ...
  },
  {
    ...
    category: "analysis",
    description: "Defines, tracks, and updates search visibility across a prioritized keyword set (25–100 high-intent queries). Tracks ranking position, movement, and volatility at both keyword and page level. Detects gains, losses, competitor displacement, cannibalization, and intent mismatch. Feeds downstream agents (content gap, decay, executor) with reliable trend data (7/30/90d). Surfaces uncertainty clearly when rankings are volatile or data is sparse.",
    purpose: "Maintain a clear, prioritized understanding of which search queries matter and how the site is performing for them",
    inputs: ["serp_worker_base_url", "serp_worker_api_key", "target_keywords", "site_domain"],
    outputs: ["sites", "generated_at", "name", "domain", "location"],
    ...
    runTriggers: ["scheduled", "manual"],
    testMode: "worker",
    secretKeyName: "SEO_SERP_Keyword",
  },
  {
    slug: "crawl_render",
  ...
  },
  {
    ...
    category: "execution",
    description: "Drafts and validates content for blogs, pages, and content refreshes. Includes quality scoring, E-E-A-T validation, and compliance checks.",
    purpose: "Generate and validate SEO-optimized content",
    inputs: ["target_keywords", "content_gaps", "page_urls", "content_rules"],
    outputs: ["ok", "service", "version", "time"],
    keyMetrics: ["drafts", "content_pieces", "qa_score", "violations"],
    ...
    // Hub-and-Spoke capabilities
    producesArtifacts: ["content.draft", "content.revision", "qa.scorecard", "qa.fix_list"],
    consumesArtifacts: ["gap.report", "comp.snapshot"],
    requiredInputs: ["target_keywords"],
    requiresRulesBundles: ["generation", "qa", "safety"],
  },
  {
    slug: "site_executor",
  ...
  },
  {
    ...
    category: "infrastructure",
    description: "Sends Email and SMS alerts including daily summaries, critical drop notifications, indexing emergencies, and approval prompts for pending changes.",
    purpose: "Send alerts via email and SMS",
    inputs: ["notification_channels"],
    outputs: ["ok", "service", "version", "time"],  // Matches /health response for smoke tests
  ...
  },
  {
    ...
    description: "Service configuration via environment variables set in the Vercel dashboard. All API keys and base URLs for worker services are stored here.",
    purpose: "Store and retrieve service configuration",
    inputs: [],
    outputs: ["vault_status"],
    keyMetrics: ["env_vars_configured"],
    commonFailures: ["missing_env_var"],
    runTriggers: ["manual"],
    testMode: "platform",
